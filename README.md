...
> Measured against **undark** (C) and **fqlite** (Java) on a shared corpus ([`docs/recovery-comparison.md`](docs/recovery-comparison.md)): on freed-page deletions it **matches undark and beats fqlite**; on dropped tables it **exceeds undark** (recovering the dropped table's own schema record too); and — **alone among the three on the tested corpus** — it never re-surfaces a live row as "deleted", a false-positive class both undark and fqlite exhibit on the no-deletion databases. Recovery has a physical ceiling: it recovers **163 of 163 *recoverable* rows** on the fixture (~81% of all 200 deleted; the remaining ~37 were physically overwritten when their pages were freed and are unrecoverable by any tool).
